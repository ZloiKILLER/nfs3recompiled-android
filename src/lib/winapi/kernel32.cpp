                 ...
                 HANDLE hObject)
{
    ...
    {
        GenericResource* resource = hObject ? app->getResource(hObject) : nullptr;
        SDL_Log("[API] CloseHandle h=%u resource=%p file=%d thread=%d",
                unsigned(hObject), static_cast<void*>(resource),
                dynamic_cast<File*>(resource) != nullptr,
                dynamic_cast<Thread*>(resource) != nullptr);
    }
    if (hObject)
                   ...
                   DWORD dwFlagsAndAttributes, HANDLE hTemplateFile)
{
    ...
    if (*file)
    {
        const HANDLE h = app->allocateResource(file);
        if (WinApplication::traceApi())
        {
            SDL_Log("[API] CreateFileA '%s' name@0x%x [%02x %02x %02x %02x] access=0x%x -> handle=%u",
                    lpFileName, unsigned(app->guestAddress(lpFileName)),
                    unsigned(x86::reg8(lpFileName[0])),
                    unsigned(x86::reg8(lpFileName[0] ? lpFileName[1] : 0)),
                    unsigned(x86::reg8(lpFileName[0] && lpFileName[1] ? lpFileName[2] : 0)),
                    unsigned(x86::reg8(lpFileName[0] && lpFileName[1] && lpFileName[2] ? lpFileName[3] : 0)),
                    unsigned(dwDesiredAccess), unsigned(h));
        }
        return h;
    }
    else
    {
        if (WinApplication::traceApi())
        {
            SDL_Log("[API] CreateFileA '%s' name@0x%x access=0x%x -> FAILED", lpFileName,
                    unsigned(app->guestAddress(lpFileName)), unsigned(dwDesiredAccess));
        }
        delete file;
        return HANDLE(-1);
    }
                          ...
                          LPCSTR lpName)
{
    ...
    if (WinApplication::traceApi())
    {
        SDL_Log("[API] CreateFileMappingA hFile=%u flProtect=0x%x", unsigned(hFile), unsigned(flProtect));
    }
                    ...
                    LPDWORD lpThreadId)
{
    ...
    const HANDLE h = app->allocateResource(thread);
    if (WinApplication::traceApi())
    {
        SDL_Log("[API] CreateThread entry=0x%x param=0x%x flags=0x%x -> h=%u tid=%u",
                unsigned(lpStartAddress), unsigned(lpParameter), unsigned(dwCreationFlags),
                unsigned(h), unsigned(thread->threadId()));
    }
    return h;
}
                          ...
                          LPCRITICAL_SECTION lpCriticalSection)
{
    NFS2_USE(app);
    NFS2_USE(cpu);
    /* An uncontended critical section -- overwhelmingly the common case, since
     * most Win32 critical sections guard something brief -- used to always pay
     * for releasing and reacquiring the single global guest execution context
     * anyway.  That context is shared by every guest thread, so on a screen
     * where more than one thread hammers a critical section at high frequency
     * (observed on device: one thread alone doing 18000+ Enter/GetTickCount/
     * Leave triples per second while the Camera/Graphics settings screens are
     * open, other threads doing the same at the same time), every single call
     * became a real OS mutex release-then-reacquire against contention from
     * every other such thread -- a thundering-herd on the context mutex, not
     * on the critical section it was nominally protecting.  Try the fast,
     * non-blocking path first; only fall back to the release-context dance
     * when the critical section is genuinely held by someone else, which is
     * exactly the case that needs it (another guest thread may itself be
     * blocked waiting for the execution context, so it cannot be allowed to
     * make progress and release the critical section unless we let go). */
    if ((*lpCriticalSection)->tryLock())
    {
        return;
    }
    app->unlockContext(cpu);
                       ...
                       LPDWORD lpNumberOfFreeClusters, LPDWORD lpTotalNumberOfClusters)
{
    ...
    if (WinApplication::traceApi())
    {
        SDL_Log("[API] GetDiskFreeSpaceA root='%s' root@0x%x spc=%u bps=%u free=%u total=%u",
                lpRootPathName ? lpRootPathName : "(null)",
                unsigned(app->guestAddress(lpRootPathName)),
                lpSectorsPerCluster ? unsigned(*lpSectorsPerCluster) : 0,
                lpBytesPerSector ? unsigned(*lpBytesPerSector) : 0,
                lpNumberOfFreeClusters ? unsigned(*lpNumberOfFreeClusters) : 0,
                lpTotalNumberOfClusters ? unsigned(*lpTotalNumberOfClusters) : 0);
    }
    return 1;
}
                  ...
                  HANDLE hFile, LPDWORD lpFileSizeHigh)
{
    ...
    if (lpFileSizeHigh)
        *lpFileSizeHigh = 0;
    const DWORD sz = file ? file->size() : DWORD(-1);
    if (WinApplication::traceApi())
    {
        SDL_Log("[API] GetFileSize hFile=%u file=%p size=%u", unsigned(hFile),
                static_cast<void*>(file), unsigned(sz));
    }
    return sz;
}
...
{
    NFS2_USE(app);
    /* Diagnostic, kept live: this rate-limit trips whenever some guest thread
     * calls GetTickCount 500+ times in 200ms, which is what a spin-wait poll
     * loop looks like from here.  Two different capture sessions have now
     * shown this pattern -- once with real disk I/O happening concurrently on
     * a *different* thread (paused-race traffic-car streaming, unrelated
     * background noise it turned out), and once, from a clean main-menu-only
     * repro of the Camera/Graphics/HUD settings hang, with NO other I/O in
     * flight at all -- so whatever this second case is polling is not file
     * read completion, at least not directly.  The backtrace below exists to
     * find out what it actually is on the NEXT repro, since guessing from the
     * first (misleading) capture already cost real time once.  Applying a
     * real sleep while throttling is a no-op that helps if this specific
     * caller is a bounded livelock and does nothing otherwise; it was tested
     * against the live hang and did NOT resolve it, ruling out "just needs
     * a bigger OS scheduling slice" as the fix, whatever the root cause is. */
    static Uint64 s_windowStartMs = 0;
    static int s_windowCalls = 0;
    static bool s_throttling = false;
    static bool s_dumped = false;
    const Uint64 now = SDL_GetTicks();
    if (now - s_windowStartMs > 200)
    {
        if (s_throttling && s_windowCalls < 200)
        {
            s_throttling = false;
            SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION, "[STREAMTHROTTLE] released");
        }
        s_windowStartMs = now;
        s_windowCalls = 0;
    }
    if (++s_windowCalls == 500 && !s_throttling)
    {
        s_throttling = true;
        SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION,
                    "[STREAMTHROTTLE] engaged: %d GetTickCount calls in 200ms", s_windowCalls);
        if (!s_dumped)
        {
            s_dumped = true;
#ifndef _WIN32
            struct Ctx { void* frames[16]; int n; };
            Ctx ctx = { {}, 0 };
            _Unwind_Backtrace(
                [](_Unwind_Context* uc, void* arg) -> _Unwind_Reason_Code
                {
                    Ctx* c = static_cast<Ctx*>(arg);
                    if (c->n < 16)
                    {
                        c->frames[c->n++] = reinterpret_cast<void*>(_Unwind_GetIP(uc));
                    }
                    return c->n < 16 ? _URC_NO_REASON : _URC_END_OF_STACK;
                },
                &ctx);
            SDL_Log("[HANGDIAG] %d backtrace frames:", ctx.n);
            for (int i = 0; i < ctx.n; ++i)
            {
                SDL_Log("[HANGDIAG]   frame %d: %p", i, ctx.frames[i]);
            }
#endif
        }
    }
                         ...
                         DWORD dwFileOffsetLow, SIZE_T dwNumberOfBytesToMap)
{
    ...
    app->lockContext(cpu);
    const x86::reg32 address = result->getBlockStart();
    if (WinApplication::traceApi())
    {
        x86::reg32 first = 0;
        if (bytesRead >= sizeof(first))
            memcpy(&first, data, sizeof(first));
        SDL_Log("[API] MapViewOfFile h=%u offset=%u requested=%u read=%u -> addr=0x%x first4=0x%08x",
                unsigned(hFileMappingObject), unsigned(dwFileOffsetLow),
                unsigned(dwNumberOfBytesToMap), unsigned(bytesRead), unsigned(address),
                unsigned(first));
    }
    return address;
}
              ...
              LPDWORD lpNumberOfBytesRead, LPOVERLAPPED lpOverlapped)
{
    ...
    if (!f)
    {
        if (WinApplication::traceApi())
        {
            SDL_Log("[API] ReadFile h=%u requested=%u overlapped=%p -> INVALID_HANDLE",
                    unsigned(hFile), unsigned(nNumberOfBytesToRead),
    ...
    else
    {
        ...
        if (WinApplication::traceApi())
        {
            const DWORD bytesRead = lpNumberOfBytesRead ? *lpNumberOfBytesRead : 0;
            DWORD first = 0;
            if (lpBuffer && bytesRead >= sizeof(first))
                memcpy(&first, lpBuffer, sizeof(first));
            SDL_Log("[API] ReadFile h=%u requested=%u overlapped=%p -> ok=%u read=%u first4=0x%08x",
                    unsigned(hFile), unsigned(nNumberOfBytesToRead),
                    static_cast<void*>(lpOverlapped), unsigned(result), unsigned(bytesRead),
                    unsigned(first));
        }
        return result;
    }
                     ...
                     HANDLE hFile, LONG lDistanceToMove, PLONG lpDistanceToMoveHigh, DWORD dwMoveMethod)
{
    ...
    const DWORD result = file ? file->seek(dwMoveMethod, lDistanceToMove) : DWORD(-1);
    if (WinApplication::traceApi())
    {
        SDL_Log("[API] SetFilePointer h=%u method=%u distance=%d -> %u",
                unsigned(hFile), unsigned(dwMoveMethod), int(lDistanceToMove), unsigned(result));
    }
    return result;
}
    ...
    Packed<const void> lpBaseAddress)
{
    ...
    if (lpBaseAddress)
    {
        ...
        if (WinApplication::traceApi())
        {
            SDL_Log("[API] UnmapViewOfFile addr=0x%x block=%p",
                    unsigned(x86::reg32(lpBaseAddress)), static_cast<void*>(memBlock));
        }
        delete memBlock;
        return 1;
    }
    ...
    {
        if (WinApplication::traceApi())
            SDL_Log("[API] UnmapViewOfFile addr=0 -> FALSE");
        return 0;
    }
}
                        ...
                        LPVOID lpAddress, SIZE_T dwSize, DWORD flAllocationType, DWORD flProtect)
{
    ...
    if (WinApplication::traceApi())
    {
        SDL_Log("[API] VirtualAlloc size=%u -> 0x%x..0x%x", unsigned(dwSize),
                unsigned(map->getBlockStart()),
                unsigned(map->getBlockStart() + map->getBlockSize()));
    }
                 ...
                 Packed<void> lpAddress, SIZE_T dwSize, DWORD dwFreeType)
{
    ...
    if (WinApplication::traceApi())
    {
        SDL_Log("[API] VirtualFree addr=0x%x -> block 0x%x..0x%x", unsigned(x86::reg32(lpAddress)),
                memBlock ? unsigned(memBlock->getBlockStart()) : 0u,
                memBlock ? unsigned(memBlock->getBlockStart() + memBlock->getBlockSize()) : 0u);
    }
    delete memBlock;
    return 1;
}
