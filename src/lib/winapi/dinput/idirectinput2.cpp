                                    ...
                                    REFGUID rguid, Packed<IDirectInputDevice>* lplpDirectInputDevice, LPUNKNOWN pUnkOuter)
{
    ...
    /* Which devices the game actually opens decides which input path matters.
     * Keyboard is the interesting one: class Keyboard (lib/gamepad.h) is empty
     * and GetDeviceState() only zeroes the buffer, so a menu reading keys this
     * way sees nothing pressed, ever -- regardless of the message queue. */
    /* Only read by the trace below, which compiles out with the option off --
     * clang -Werror catches that, MSVC does not. */
    NFS2_USE(kind);
    NFS_MSG_TRACE("dinput CreateDevice %s", kind);
